...
import io  # ← 追加
import json as json_lib  # ← 追加import discord
from discord.ext import commands, tasks
from discord import app_commands
from discord.ui import View, Button
from typing import Optional, List, Union
...
async def run_unit_tests() -> list[str]:
    """重要なロジック関数の単体テストを実行"""
    results = []
    ...
    try:
        ...
        assert normalize_text("全角　スペース") == "全角スペース"
        results.append("✅ UnitTest: normalize_text OK")
    except AssertionError:
        results.append("❌ UnitTest: normalize_text FAILED")
    except Exception as e:
        results.append(f"❌ UnitTest: normalize_text Error: {e}")

    # テスト2: is_authorized (モック不要な範囲で)
    try:
        # OWNER_ID等は環境依存だが、ロジック自体の確認
        assert is_authorized(OWNER_ID) is True
        results.append("✅ UnitTest: is_authorized OK")
    except AssertionError:
        results.append("❌ UnitTest: is_authorized FAILED")
    
    # テスト3: has_any
    try:
        assert has_any("こんにちは", ["あいさつ", "こんにち"]) is True
        assert has_any("さようなら", ["あいさつ", "こんにち"]) is False
        results.append("✅ UnitTest: has_any OK")
    except AssertionError:
        results.append("❌ UnitTest: has_any FAILED")

    # テスト4: normalize_synonyms
    try:
        assert normalize_synonyms("オートピング") == "autoping"
        assert normalize_synonyms("BANして") == "出禁して"
        results.append("✅ UnitTest: normalize_synonyms OK")
    except AssertionError:
        results.append("❌ UnitTest: normalize_synonyms FAILED")
...
async def run_daily_test(channel):
    ...
    try:
        ...
        # 0. 単体テスト実行 (追加)
        unit_test_results = await run_unit_tests()
        results.extend(unit_test_results)
...
def validate_settings() -> bool:
    ...
    # 1. 必須環境変数
    if OWNER_ID == 0:
        issues.append("❌ Critical: OWNER_ID が設定されていません (環境変数)")
    
    # 2. 設定ファイルのデータ型チェック
    if not isinstance(ADMIN_IDS, set):
        issues.append(f"❌ Error: ADMIN_IDS の型が不正です ({type(ADMIN_IDS)})")
    if not isinstance(BLOCKED_USERS, set):
        issues.append(f"❌ Error: BLOCKED_USERS の型が不正です ({type(BLOCKED_USERS)})")
    if not isinstance(TARGET_VC_IDS, set):
        issues.append(f"❌ Error: TARGET_VC_IDS の型が不正です ({type(TARGET_VC_IDS)})")
    if not isinstance(vc_block_enabled, bool):
        issues.append(f"❌ Error: vc_block_enabled の型が不正です ({type(vc_block_enabled)})")
    
    # 3. Google API設定 (Warning)
    if not google_service:
        issues.append("⚠️ Warning: Google Custom Search API が未設定です (検索機能が無効)")
    if not vision_client:
        issues.append("⚠️ Warning: Google Vision API が未設定です (画像認識が無効)")
        
    # 4. チャンネルIDの整合性 (簡易チェック)
    if AUTO_PING_CHANNEL_ID != 0 and AUTO_PING_CHANNEL_ID < 10000000000000000:
         issues.append(f"⚠️ Warning: AUTO_PING_CHANNEL_ID が不正な値の可能性があります ({AUTO_PING_CHANNEL_ID})")
    ...
    if issues:
        ...
        for issue in issues:
            print(issue)
        print("========================================")
        # Criticalが含まれていたらFalseを返すなどの判定も可能だが、
        # 現状はログ出力にとどめ、Trueを返す（Botを止めないため）
    else:
        print("✅ 設定バリデーション: OK (全項目正常)")
